...
def echo(str):
	str=str.replace('\\', '\\\\')
	str=str.replace('"', "'")
	vim.command("redraw | echomsg \"%s\"" % str)
...
class AutoTag:
	...
	def findTagFile(self, source):
		...
		( drive, filepath ) = os.path.splitdrive(source)
		dir_name = os.path.dirname(filepath)
		level_search = 8
		while True:
			#self.__diag("dir_name = %s" % dir_name)
			#self.__diag('drive = "%s", file = "%s"' % (drive, file))
			tagsFile = os.path.join(drive, dir_name, self.tags_file)
	...
	def addSource(self, source):
		...
		if tagsFile:
			...
			if self.tags.has_key(tagsFile):
				...
				self.__diag("no tags files found")
